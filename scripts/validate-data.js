...
if (wp) {
  if (!Array.isArray(wp)) {
    console.error('FAIL writing-prompts: expected array at top level');
    errors++;
  } else {
...
if (pg) {
  const plotGenres = ['fantasy','scifi','mystery','romance','thriller','literary','comedy'];
  ...
  for (const comp of components) {
    ...
    // Check genre tags
    check(`plot[${comp}] genre`, items, i => i.genre, plotGenres);
  }
  ...
  if (templates.length < 4) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const result = t
        ...
        .replace('{theme}', (pick(pg.themes || []) || {}).text || '?');
