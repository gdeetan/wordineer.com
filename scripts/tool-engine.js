...
const liveAugmentedTypes = new Set();
function scheduleLiveAugment(type = 'all', delay = 0) {
const key = type || 'all';
if (liveAugmentedTypes.has(key)) return;
liveAugmentedTypes.add(key);
const run = () => augmentLive(key).catch(() => {});
setTimeout(() => {
